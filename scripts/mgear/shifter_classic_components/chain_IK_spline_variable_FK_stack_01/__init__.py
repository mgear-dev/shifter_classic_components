...
class Component(component.Main):
    ...
    def addObjects(self):
        ...
        for i in range(self.settings["fkNb"]):
            ...
            if i in [0, self.settings["fkNb"] - 1] and False:
                ...
                #     fk_npo, self.getName("fk%s_global_npo" % i), global_t)
                # fk_global_in = primitive.addTransform(
                #     fk_global_npo,
                #     self.getName("fk%s_global_in" % i),
                #     global_t)
                # self.fk_global_in.append(fk_global_in)

                # local input
                # Note: local will be parent later to global after alignment
                # is finish in the addOperators method
                # this is a dirty work around since we dont calculate the
                # final positions at creation time
                ...
                #     fk_npo,
                #     self.getName("fk%s_global_out_npo" % i),
                #     global_t)
    ...
    def addOperators(self):
        ...
        # update fk parenting
        # for e, g_in in enumerate(self.fk_global_in):
        #     t = transform.getTransform(g_in)
        #     global_t = transform.setMatrixPosition(
        #         datatypes.Matrix(),
        #         transform.getPositionFromMatrix(t))
        #     g_in.getParent().setMatrix(global_t, worldSpace=True)
        #     # self.fk_global_out[e].getParent().setMatrix(global_t,
        #     #                                             worldSpace=True)
        #     # self.fk_global_ref[e].getParent().setMatrix(global_t,
        #     #                                             worldSpace=True)
        #     pm.parent(self.fk_local_in[e].getParent(), g_in)
